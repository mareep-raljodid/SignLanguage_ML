...
def FusedConv2DBiasActivation(conv_input, filter, bias, side_input, conv_input_scale, side_input_scale, strides, padding, data_format="NHWC", filter_format="HWIO", activation_mode="Relu", dilations=[1, 1, 1, 1], name=None):
  return fused_conv2d_bias_activation(conv_input=conv_input, filter=filter, bias=bias, side_input=side_input, conv_input_scale=conv_input_scale, side_input_scale=side_input_scale, strides=strides, padding=padding, data_format=data_format, filter_format=filter_format, activation_mode=activation_mode, dilations=dilations, name=name)
FusedConv2DBiasActivation.__doc__ = fused_conv2d_bias_activation.__doc__
FusedConv2DBiasActivation = _doc_controls.do_not_generate_docs(_kwarg_only(FusedConv2DBiasActivation))
